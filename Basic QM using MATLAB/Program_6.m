...
% Parameters for making intial momentum space wavefunctionphi(k)
ko = 2;                      % Peak momentum
...
phi = exp(-a*(k-ko).^2).*exp(-1i*6*k.^2); % unnormalizedphi(k)
...
for t = 1:NT                % time index for loop
    ...
    axis([0 L 0 0.25]);     % set x,y axis parameters for plotting
    xlabel('x [m]', 'FontSize', 24);ylabel('probability density [1/m]','FontSize', 24);
