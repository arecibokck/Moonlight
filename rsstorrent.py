...
# to own.
FEEDS = [
	   "https://showrss.info/user/48490.rss?magnets=true&namespaces=true&name=clean&quality=hd&re=null",
	]
	
DOWNLOAD_DIR = "F:\TV Shows"
TIMESTAMP    = "F:\TV Shows\rsstorrent.stamp"
VERBOSE      = True
...
def download(url):
    ...
    to a local file.
    """
    remote_file = urllib.parse.urlopen(url)
...
for feed_url in FEEDS: 
    ...
    if feed["bozo"] != 1:
        for item in feed["items"]:
            items.append((item["updated_parsed"], item))
    else:
        if VERBOSE:    
